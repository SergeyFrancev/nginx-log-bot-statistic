...
robot_useragents = [
    ...
    'echo!',
    'fast\-webcrawler',
    # Must be before ia_archiver to avoid confusion with alexa
    ...
    # must come before fast enterprise crawler
    ...
    # New bots
    'CMC/0.01', 'christcrawler', 'BotLink', 'googlebot', 'Nederland.zoek', 'WOLP', 'crawlpaper', 'Pimptrain', 'conceptbot', 'arks', 'Ask Jeeves', 'boxseabot', 'topiclink', 'searchprocess', 'SimBot', 'ahoy', 'Senrigan', 'speedy', 'suke', 'AlkalineBOT', 'snooper', 'cusco', 'legs', 'spiderline', 'Search-AU', 'jcrawler', 'webs', 'VWbot_K', 'dwcp', 'WWWC', 'cyberspyder', 'roadrunner', 'robots.txt', 'irobot', 'moget', 'griffon', 'robofox', 'urlck', 'fido', 'KIT-Fireball', 'MindCrawler', 'araneo', 'Orbsearch/1.0', 'elfinbot', 'muninn', 'phpdig', 'larbin', 'wapspider', 'Iron33', 'MwdSearch', 'JBot', 'Site Valet', 'ariadne', 'TechBOT', 'AraybOt', 'Robbie', 'esculapio', 'MerzScope', 'bbot', 'INGRID/0.1', 'PortalBSpider', 'borg-bot/0.9', 'webquest', 'grabber', 'muncher', 'Internet Cruiser Robot', 'Bjaaland', 'gulliver', 'void-bot', 'desertrealm, desert realm', 'cydralspider', 'Valkyrie libwww-perl', 'FELIX IDE', 'NEC-MeshExplorer', 'gulper', 'Freecrawl', 'bspider', 'htdig', 'Shagseeker', 'SLCrawler/2.0', 'T-H-U-N-D-E-R-S-T-O-N-E', 'verticrawl', 'SpiderBot/1.0', 'XGET', 'havIndex', 'RoboCrawl', 'msnbot', 'sift', 'ParaSite', 'wlm', 'skymob', 'CoolBot', 'MuscatFerret', 'ObjectsSearch', 'WebBandit/1.0', 'Voyager', 'hotwired', 'iajabot', 'Jeeves', 'calif', 'ATN_Worldwide', 'Informant', 'webreaper', 'WebWalker', 'Raven', 'webcatcher', 'Motor', 'tach_bw', 'esther', 'marvin', 'downloadexpress', 'perlcrawler', 'DragonBot', 'WebMoose', 'gammaSpider', 'image.kapsi.net', 'linkwalker', 'mediafox', 'ko_yappo_robot', 'sharp-info-agent', 'WebMechanic', 'psbot', 'slurp', 'ecollector', 'winona', 'combine', 'DIIbot', 'nil', 'Scooter', 'FDSE', 'ESI', 'appie', 'KDD-Explorer', 'Victoria', 'mattie', 'newscan-online', 'PlumtreeWebAccessor', 'webspider', 'Atomz', 'InfoSpiders', 'Hämähäkki', 'RHCS', 'fouineur', 'Getterrobo-Plus', 'Ukonline', 'cosmos', 'label-grabber', 'pjspider', 'spider_monkey', 'Teoma', 'Gromit', 'wget', 'confuzzledbot', 'Open Text Site Crawler', 'Linkidator', 'Lockon', 'pageboy', 'templeton', 'RixBot', 'Spanner', 'anthill', 'NetScoop', 'patric', 'whowhere', 'inspectorwww', 'titin', 'piltdownman', 'gcreep', 'solbot', 'Occam', 'logo_gif_crawler', 'http://www.sygol.com', 'pegasus', 'jobo', 'hambot', 'robi', 'ebiness', 'arale', 'packrat', 'gazz', 'udmsearch', 'fastcrawler', 'UdmSearch', 'golem', 'digger'
]
    ...
    for line in lines:
        if line.startswith("robot-exclusion-useragent:"):
            ...
            if ' or ' in ua:
                uas = ua.split(" or ")
                # remove quotes
                uas = [x[1:-1] if (x[0] in ['"', "'"] and x[-1]
                                   in ['"', "'"]) else x for x in uas]
            else:
                uas = [ua]
            for ua in uas:
...
def check_rule(user_agent):
    agent = user_agent.lower()
    for robot_ua in robot_useragents:
        if robot_ua.search(agent):
            print('Agent is Bot by:', robot_ua)
            return
    print('Agent is not Bot')
...
if __name__ == '__main__':
    if len(sys.argv) == 2:
        ...
        command = sys.argv[1]
        if command == 'check_rule':
            check_rule(sys.argv[2])
